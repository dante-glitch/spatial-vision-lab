...
def evaluate(
    ...
    print("=" * 60)
    print(f"  Registered cameras       : {sfm_map.n_cameras}")
    print(f"  Cameras with GT          : {n_gt_cameras}")
    print(f"  Total 3D points          : {sfm_map.n_points}")
    if pose_metrics is not None and pose_metrics.get("longest_segment") is not None:
        segment = pose_metrics["longest_segment"]
        print(
            f"  Eval segment             : {segment['start_name']} -> "
            f"{segment['end_name']} (length={segment['length']})"
        )
    ...
    if pose_metrics is not None:
        ...
        print(f"\n  Rotation error after alignment (deg)")
        print(f"    mean  : {np.mean(rot_errs):.3f}")
        print(f"    median: {np.median(rot_errs):.3f}")
        print(f"    max   : {np.max(rot_errs):.3f}")

        print(f"\n  Position error after alignment")
        print(f"    mean  : {np.mean(pos_errs):.5f}  [scene units]")
        print(f"    median: {np.median(pos_errs):.5f}  [scene units]")
        print(f"    max   : {np.max(pos_errs):.5f}  [scene units]")

        print(f"\n  ATE RMSE (after Umeyama): {pose_metrics['ate_rmse']:.5f}  [scene units]")
        print(f"  Alignment scale          : {pose_metrics['alignment_scale']:.5f}")
    else:
        print("\n  Aligned pose metrics     : N/A (< 3 GT cameras)")
...
def visualize_trajectories(
    ...
    if traj is None:
        print("[Viz] Not enough GT cameras to visualise aligned trajectories.")
        return
    ...
    print("[Viz] Trajectory colors: estimated=red, ground truth=blue")
    print(f"[Viz] Umeyama alignment: {'enabled' if traj['used_umeyama'] else 'disabled'}")
    if not traj["used_umeyama"]:
        print(
            "[Viz] Scale without Umeyama: "
            f"{'enabled' if traj['scale_without_umeyama'] else 'disabled'} "
            f"(scale={traj['alignment_scale']:.6g})"
        )
    est_span = _trajectory_span(traj["est_aligned"])
    gt_span = _trajectory_span(traj["gt"])
    print(
        f"[Viz] Trajectory span: est={est_span:.6g}, "
        f"gt={gt_span:.6g}"
    )
    if est_span < 1e-9 and len(traj["est_aligned"]) > 1:
        print("[Viz] Estimated camera centers are nearly identical; check PnP pose updates.")
    print(
        f"[Viz] First matched frame: {traj['camera_names'][0]}, "
        f"est={traj['est_aligned'][0]}, gt={traj['gt'][0]}"
    )
